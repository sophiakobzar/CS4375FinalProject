...
# Define flag for error handling
flag = True
...
while True:
    ...
    # If the user chooses a valid model
        ...
        # Ask the user if they want to test their own email
        while flag:
            print("Do you want to test your own email (Yes or No)?") # test choiceEmail
            choiceEmail = input("")
            if choiceEmail == "Y": 
                print("Enter the subject of the email and then click submit.")
                print("Example input (without quotes): 'Subject: I am skydiving'")
                subject = input("")
                flag = False
            if choiceEmail == "N": 
                flag = False
        ...
        # If the user chose to test their own email
        if choiceEmail == "Y":
            subject_features = vectorizer.transform([subject])
            prediction = model.predict(subject_features) # predict based on trained model
            if prediction == 0:
                print("Email detected as Ham.\n")
            else:
                print("Email detected as Spam.\n")
